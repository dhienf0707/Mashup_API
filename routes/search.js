    ...
    let getItems = function(url) {
        return axios.post(eBayTokenAPI.url, qs.stringify(eBayTokenAPI.requestBody), {
            ...
            .then(tokenRes => {
                ...
                return Promise.all([tokenRes, result]);
            })
            .then(([tokenRes, result]) => result.data.itemSummaries)
    }
    return {
        getItems: getItems
    }
})(); 
eBayAPI.getItems('https://api.ebay.com/buy/browse/v1/item_summary/search?q=drone&limit=3')
